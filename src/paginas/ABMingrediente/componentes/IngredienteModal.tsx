...
const handleDelete = async (ingr: Ingrediente) => {
    ...
    //CREATE - UPDATE
    const handleSaveUpdate = async (ingrediente: Ingrediente) => {
        try {
            const isNew = ingrediente.id === 0;
            if (isNew) {
                await IngredienteService.createProduct(ingrediente);
            } else {
                await IngredienteService.updateProduct(ingrediente.id, ingrediente);
            }
            onHide();
